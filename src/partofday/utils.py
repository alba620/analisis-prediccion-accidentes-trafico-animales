...
def read():
    '''
    It returns an array with data to calculate part of day
    parameters:
    returns:
        array: date_accident
    '''
    ...
    try:
        ...
        if len(rows) > 0:
            for row in rows:
                ...
                date_accident.append(datetime.datetime.strptime(row[2], '%H:%M').time())   
...
def update_partofday(date_accident):
    '''
    It calculates part of day and updates parte_dia field in accidentes_animales_final
    parameters:
        date_accident: array
    returns:
        True
    ...
    for id_num, date, hour, lon, lat, nombre_ccaa in date_accident:
        ...
        try:
            sql= f"""
               update accidentes_animales_final set parte_dia = '{parte_dia}' where id_num = {id_num}
            """
            cur= connect.cursor()
            cur.execute(sql)
            connect.commit()
