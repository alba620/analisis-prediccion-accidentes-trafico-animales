...
'''
Update table accidentes_animales_final with imd data
'''
import logging
import utils
...
def update_record(id_num, imd_total):
    '''
    It updates field of 'imd_total' of each incidence record in accidentes_animales_final table
    parameters:
        id_num: int
        imd_total: int
    returns:
    ...
    try:
        sql= f"""
            update accidentes_animales_final set imd_total = {imd_total} where id_num = {id_num}
        """
        cur= connect.cursor()
        cur.execute(sql)
        connect.commit()
