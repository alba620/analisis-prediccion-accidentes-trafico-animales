...
'''
Update table accidentes_animales_final with pendiente data
'''
import logging
import psycopg2
import os
...
def update_slopes(incidences):
    '''
    It updates 'pendiente' in accidentes_animales_final table
    parameters: 
        incidences: array
    returns:
    '''
    ...
    for inci in incidences:
        ...
        # Get the sheet 
        logging.info(f"antes get_hoja")
        num_sheet = utils.get_hoja(lon, lat)
        logging.info(f"antes get_datos_hoja")
        sheet_data = utils.get_datos_hoja(num_sheet)
        logging.info(f"adespuestes get_datos_hoja")
    ...
    try:
        ...
        for row in rows:
            ...
            for row_id in rows_ids:
                ...
                logging.info(f"Processing sheet = {hoja} id = {id_num}")
                try:
                    # Reproject lon, lat of our position to X, Y of sheet
                    in_proj = Proj(init='epsg:4326') 
                    out_proj = Proj(init=f'epsg:{sheet_data.epsg}')
